...
def main():
    ...
    print("=" * 78)
    # ВНИМАНИЕ: первая версия этого расчёта была неверной. Там требуемая частота
    # дополнительно делилась на 0.020, как будто спайки считаются в окне 20 мс.
    # Лишний множитель завышал требования в пятьдесят раз, и из-за него был
    # сделан вывод, что зрение через оптические доли невозможно в принципе.
    #
    # Правильный расчёт — установившийся режим:
    #   проводимость   g_ss = (tauSyn/dt) * wScale * Σw * r * 1e-4 = 0.001375 * Σw * r
    #   потенциал      v - v_rest = g_ss
    #   порог          7 мВ  ->  Σw * r = 5091
    # Отсюда требуемая частота входа r = 5091 / Σw, без всяких окон.
    print("  Установившийся режим: g_ss = 0.001375 * Σвес * частота, а в равновесии")
    print("  v - v_rest = g_ss. До порога нужно 7 мВ, значит Σвес * частота = 5091.")
    print("  Требуемая частота на входах равна 5091 / Σвес.\n")

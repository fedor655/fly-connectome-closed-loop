...
def main():
    ...
    print("=" * 78)
    rel_half = float(np.mean([corr(results[k]["dev0"], results[k]["dev1"])
                              for k in names]))
    # Половина окна вдвое шумнее целого, поэтому сравнивать корреляции сцен
    # (они считаны по целому окну) с надёжностью по половинам напрямую нельзя.
    # Поправка Спирмена-Брауна приводит надёжность к длине целого окна.
    rel = 2 * rel_half / (1 + rel_half)
    same = df[df["label"].isin(["две левых", "две правых"])]["corr_dev"].mean()
    diff = df[df["label"] == "лево против право"]["corr_dev"].mean()
    print(f"  надёжность по половинам окна:        {rel_half:.3f}")
    print(f"  она же в пересчёте на целое окно:    {rel:.3f}  <- ПОТОЛОК")
    print(f"  корреляция внутри одной стороны:     {same:.3f}")
    print(f"  корреляция между сторонами:          {diff:.3f}")
    print(f"\n  с поправкой на шум (делим на потолок):")
    print(f"    внутри одной стороны {same / max(rel, 1e-9):+.2f}, "
          f"между сторонами {diff / max(rel, 1e-9):+.2f}")

    print("\n" + "-" * 78)
    print("  Читать так. Потолок — это корреляция сцены с САМОЙ СОБОЙ: выше него")
    print("  не бывает ничего. Если бы две сцены были для мухи одинаковы, они бы")
    print("  дали ровно потолок.")
    ok_side = abs(diff) > 0.5 * rel
    ok_within = same < 0.5 * rel
    print(f"\n  сторона объекта закодирована: {'да' if ok_side else 'НЕТ'} "
          f"(|{diff:.3f}| против потолка {rel:.3f})")
    print(f"  место ВНУТРИ стороны различается: {'да' if ok_within else 'НЕТ'} "
          f"({same:.3f} заметно ниже потолка {rel:.3f}, то есть две сцены одной")
    print(f"  стороны для мухи НЕ одинаковы; будь код только по яркости глаза,")
    print(f"  они дали бы потолок)")
    print(f"\n  Но сам потолок низкий ({rel:.2f}): картина LC за одно окно сильно")
    print("  зашумлена счётом спайков. Для поведения это не приговор — команда")
    print("  снимается с популяции нисходящих и сглаживается, — но заявлять по")
    print("  этим числам тонкое различение мест нельзя.")
